"""
Task 2: LUAD vs LUSC Classification (v3 — Virchow2 features)
=============================================================
Uses pre-extracted Virchow2 pathology foundation model features.
No feature extraction stage — loads existing .pt files directly.
"""
...
# ============================================================================
# Gated Attention MIL (2560-dim Virchow2 input)
# ============================================================================
...
# ============================================================================
# Virchow2 Feature Dataset
# ============================================================================
class Virchow2Dataset(Dataset):
    """Loads pre-extracted Virchow2 features and samples random bags."""
    ...
    def __init__(self, df, feature_dir, label_map, bag_size=256, augment=False):
        ...
        # Build filename → file_path mapping once
        self._build_index()

    def _build_index(self):
        """Map each CSV row to its Virchow2 feature file."""
        self.file_map = {}
        missing = 0
        for idx, row in self.df.iterrows():
            slide_name = row['filename']
            fpath = _find_virchow2_file(slide_name, self.feature_dir)
            if fpath:
    ...
    def __getitem__(self, idx):
        ...
        if fpath is None:
            ...
            features = torch.load(fpath, weights_only=True)  # (N, 2560)
...
def _find_virchow2_file(slide_name, feature_dir):
    """Match CSV slide name to Virchow2 .pt file."""
    stem = os.path.splitext(slide_name)[0]  # remove .svs

    # Try LUAD__ / LUSC__ prefix (TCGA format)
    for prefix in ['LUAD__', 'LUSC__']:
        fpath = os.path.join(feature_dir, prefix + stem + '.pt')
        if os.path.exists(fpath):
            return fpath
    # Try without prefix (Nanfang format)
...
# ============================================================================
# Data Loading (from original task2)
# ============================================================================
...
def test_model(model, dataloader, device):
    ...
    plt.title('Confusion Matrix - NSCLC (Virchow2)')
    ...
    if fpr is not None:
        ...
        plt.title('ROC Curve - NSCLC (Virchow2)')
...
def main():
    ...
    # Build datasets (no feature extraction needed!)
    print("\nLoading Virchow2 features...")
    train_dataset = Virchow2Dataset(train_df, CONFIG['tcga_features'], label_map,
                                     bag_size=CONFIG['bag_size'], augment=True)
    val_dataset = Virchow2Dataset(val_df, CONFIG['tcga_features'], label_map,
                                   bag_size=CONFIG['bag_size'], augment=False)
    test_dataset = Virchow2Dataset(nanfang_df, CONFIG['nanfang_features'], label_map,
                                    bag_size=CONFIG['bag_size'], augment=False)
    ...
    print("\n" + "=" * 50)
    print("STAGE: Nanfang Test Evaluation")
    print("=" * 50)
