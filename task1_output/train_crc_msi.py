...
def generate_heatmaps(model, image_paths, data_path, device):
    ...
    for target_img in image_paths:
        ...
        csv_path = os.path.join(data_path, 'label.csv')
        df = pd.read_csv(csv_path)
        matching = df[df['image_path'].str.contains(target_img.split('/')[-1].split('_')[0])]
        
        if len(matching) > 0:
            img_path = os.path.join(data_path, matching.iloc[0]['image_path'])
            ...
            if os.path.exists(img_path):
                ...
                filename = target_img.replace('/', '_').replace('.jpg', '_heatmap.png')
                plt.tight_layout()
...
def main():
    ...
    data_path = '/jhcnas7/Pathology/PathLab_data_collection/Data/CRC-MSI'
    ...
    print("\nGenerating heatmaps...")
    generate_heatmaps(model, target_images, '/jhcnas7/Pathology/PathLab_data_collection/Data/CRC-MSI', device)
