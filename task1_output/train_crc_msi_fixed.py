...
def load_data(data_path):
    ...
    if os.path.exists(csv_path):
        ...
        # Save generated label.csv to local directory (data path is read-only)
        local_csv = 'label.csv'
        df.to_csv(local_csv, index=False)
        print(f"Generated {local_csv} from directory structure (saved locally)")
...
def generate_heatmaps(model, image_paths, data_path, device):
    ...
    for target_img in image_paths:
        # Search for the matching image by scanning the directory structure
        img_path = None
        target_stem = target_img.split('/')[-1].split('_')[0]
        ...
        if img_path is None or not os.path.exists(img_path):
            print(f"Could not find matching image for: {target_img}")
            continue
