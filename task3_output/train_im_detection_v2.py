...
class GatedAttentionMIL(nn.Module):
    ...
    def forward(self, features):
        ...
        aggregated = torch.sum(A * features, dim=1)
...
def pre_extract_all_features(df, data_path, feature_extractor, device, cache_dir):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Extracting features"):
        ...
        if result is None or result.shape[0] < CONFIG['n_bag_size']:
            ...
            if result is None:
                print(f"  WARNING: Could not extract for {slide_name}, using random features")
            else:
                print(f"  WARNING: Only {actual} patches for {slide_name}, padding")
